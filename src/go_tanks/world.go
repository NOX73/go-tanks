...
func NewWorld(config WorldConfig, name string) *World {
...
func (w *World) run() {
	go w.start()
	log.Server("World started. Name: ", w.Name)
}
...
func (w *World) processCommand(m *i.Message, client i.Client) {
	message := *m
	if client == nil {
		client = message["Client"].(i.Client)
	}
...
func (w *World) processTankCommand(m *i.Message, client i.Client) {
	...
	if message["LeftMotor"] != nil {
		tank.LeftMotor = message["LeftMotor"].(float64)
	}
	if message["RightMotor"] != nil {
		tank.RightMotor = message["RightMotor"].(float64)
	}
	if message["Fire"] != nil {
		w.fireTank(tank)
	}
	if message["Gun"] != nil {
		w.processGunCommand(m, client)
	}
}
...
func (w *World) removeClient(client i.Client) {
	...
	if index < 0 {
		log.Warning("Client hasn't been removed.")
		return
	}
